  ...
  async evaluateGame(inputRaw) {
    ...
    if (!oddsList.length) {
      return {
        ...
        pick: input.team, // still provide pick (user team) even when no data
        flags: ["no_odds_found"],
    ...
    const finalConfidence = Math.round(fused * 1000) / 10; // percent with 1 decimal

    // Determine pick (always provide a pick: back the user's team)
    const pick = input.team;

    // Determine textual decision and low-confidence flag behavior
    let decisionLabel;
    ...
    if (finalConfidence >= this.thresholds.LOCK_CONFIDENCE * 100) {
      ...
      // Instead of returning PASS, return a labeled low-confidence lean so caller always gets an actionable pick
