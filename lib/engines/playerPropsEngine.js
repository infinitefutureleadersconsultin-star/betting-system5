      ...
      if (this.apiClient && this.apiClient.apiKey) {
        ...
        try {
          if (sport === "NFL") {
            ...
            // Use direct player game logs for MLB (more efficient than date-based search)
            if (sport === "MLB" && this.apiClient && typeof this.apiClient.getMLBPlayerGameLogs === "function") {
              try {
                const gameLogs = await _promiseWithTimeout(
                  this.apiClient.getMLBPlayerGameLogs(input.player, seasonYear, 20),
                  15000,
                ...
                if (Array.isArray(gameLogs) && gameLogs.length > 0) {
                  ...
                  console.log(`[PlayerPropsEngine] MLB game logs fetched: ${recentVals.length} games for ${input.player}`);
                } else {
                  // Fallback to date-based search if direct fetch fails
                  recentVals = await this._collectRecentByDate(input, sport, dateStr, 15, 10, idHint);
                }
              } catch (err) {
                console.warn("[PlayerPropsEngine] MLB game logs failed, using fallback:", err?.message);
                recentVals = await this._collectRecentByDate(input, sport, dateStr, 15, 10, idHint);
              }
            } else {
              recentVals = await this._collectRecentByDate(input, sport, dateStr, 15, 10, idHint);
