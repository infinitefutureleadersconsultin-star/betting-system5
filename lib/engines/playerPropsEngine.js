...
export class PlayerPropsEngine {
  ...
  async evaluateProp(input) {
    ...
    return {
      ...
      prop: input.prop,
      decision,
      finalConfidence: 65, // placeholder confidence
      suggestion: decision === "OVER" ? "Bet Over" : decision === "UNDER" ? "Bet Under" : "Skip",
      suggestedStake: 2, // % bankroll
      topDrivers: [
        `Recent avg = ${avg}`,
        `Line = ${line}`,
      ],
      ...
      rawNumbers: {
        avgRecent: avg,
        line,
        sampleSize: features.recentSample.length,
      },
      meta: { usedEndpoints: this.usedEndpoints }
    };
  }
}
