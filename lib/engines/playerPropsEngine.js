...
// PRODUCTION OPTIMIZED: Reduced lookback from 120 days to 15 days (90% API cost reduction)
  ...
  _pickValueFromRow(sport, prop, row) {
    try {
      ...
      if (s === "MLB") {
        ...
        // TODO: Add more MLB prop types (hits, runs, RBIs, home runs)
        return NaN;
      }
      ...
      if (s === "NBA" || s === "WNBA") {
        ...
        // TODO: Add steals, blocks, threes, turnovers
        return NaN;
      }
      ...
      if (s === "NFL") {
        ...
        // TODO: Add touchdowns, completions, interceptions, receptions
        return NaN;
      }
      
      // FUTURE SPORTS FRAMEWORK:
      // if (s === "NHL") {
      //   if (p.includes("goal")) return Number(row?.Goals);
      //   if (p.includes("assist")) return Number(row?.Assists);
      //   if (p.includes("save")) return Number(row?.Saves);
      //   return NaN;
      // }
      
      // if (s === "SOCCER") {
      //   if (p.includes("goal")) return Number(row?.Goals);
      //   if (p.includes("shot")) return Number(row?.ShotsOnGoal);
      //   return NaN;
      // }
      ...
      if (this.apiClient && this.apiClient.apiKey) {
        ...
        try {
          if (sport === "NFL") {
            ...
            // CRITICAL OPTIMIZATION: Reduced from 120/45 days to 15 days
            // This reduces API calls by 87.5% for MLB and 66.7% for NBA/WNBA
