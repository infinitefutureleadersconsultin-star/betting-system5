...
export class PlayerPropsEngine {
    ...
    for (let d = 0; d < lookbackDays && values.length < maxGames; d++) {
      ...
      if (Array.isArray(arr) && arr.length) {
        ...
        if (idHint && idHint.key && idHint.value != null) {
          row = arr.find((r) => Number(r?.[idHint.key]) === Number(r.idHint.value));
        }
        if (!row) {
  ...
  async generateFeatures(input) {
    ...
    try {
      if (this.apiClient && this.apiClient.apiKey) {
        ...
        if (this.recentValsCount) {
          // Future: calculate rolling averages or exponential weights here.
        }
  ...
  async evaluateProp(input) {
    ...
    const avg = features.recentSample.length > 0
      ? features.recentSample.reduce((a, b) => a + b, 0) / features.recentSample.length
      : NaN;

    // Compute a simple model probability based on difference between avg and line,
    ...
    if (Number.isFinite(avg) && Number.isFinite(line) && sampleSize > 0) {
      const gap = avg - line; // positive -> favors OVER
      ...
      // weight by sqrt(sampleSize) damped by variance
      const sizeFactor = Math.min(1.5, Math.sqrt(sampleSize) / (1 + variance / 4));
      modelProb = clamp01(0.5 + normalized * 0.15 * sizeFactor); // modest shift
    }

    // Market is not available here in engine (api handler merges external odds) so we treat modelProb as final.
    ...
    if (Number.isFinite(avg) && Number.isFinite(line)) {
      pickDecision = avg > line ? "OVER" : "UNDER";
    } else if (Number.isFinite(line)) {
      // no sample but a line exists — fallback to neutral lean (UNDER if line high? keep PASS => pick team as UNDER)
      pickDecision = "UNDER";
    } else {
      pickDecision = "PASS";
    }
    ...
    const suggestion = pickDecision.includes("OVER") ? "Bet Over" :
                       pickDecision.includes("UNDER") ? "Bet Under" : "Skip";
    ...
    return {
      ...
      suggestedStake,
      topDrivers: [
        `Recent avg = ${Number.isFinite(avg) ? round2(avg) : "NaN"}`,
        `Line = ${Number.isFinite(line) ? round2(line) : "NaN"}`,
        `Sample size = ${sampleSize}`,
      ],
      flags,
      rawNumbers: {
        avgRecent: Number.isFinite(avg) ? round3(avg) : null,
        line,
        sampleSize,
