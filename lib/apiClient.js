...
function resolveEnvApiKey() {
  // ← PRIORITY: Check SPORTSDATA_API_KEY first (your main API key)
  const names = [
    "SPORTSDATA_API_KEY",  // ← YOUR MAIN KEY - CHECKED FIRST
    "SPORTS_DATA_IO_KEY",
  ...
  for (const n of names) {
    if (process.env[n] && String(process.env[n]).trim() !== "") {
      console.log(`[SportsDataIOClient] Using API key from: ${n}`);
      return String(process.env[n]).trim();
    }
  }
  console.warn("[SportsDataIOClient] No API key found in environment variables");
  return "";
}
...
async function _detectMLBSeason(client) {
  try {
    ...
    console.log(`[_detectMLBSeason] Trying season: ${currentYear}`);
    ...
    console.log(`[_detectMLBSeason] Current year empty, trying: ${prevYear}`);
    ...
    console.log(`[_detectMLBSeason] Both failed, fallback to: ${prevYear}`);
    return prevYear;
  } catch (err) {
    const fallback = new Date().getFullYear() - 1;
    console.warn(`[_detectMLBSeason] Error: ${err?.message}, fallback to season: ${fallback}`);
    return fallback;
  }
}
...
export class SportsDataIOClient {
  constructor(opts = {}) {
    ...
    if (!this.apiKey) {
      console.error("[SportsDataIOClient] ⚠️ WARNING: No API key configured! Set SPORTSDATA_API_KEY in environment variables.");
    } else {
      console.log(`[SportsDataIOClient] Initialized with API key: ${this.apiKey.substring(0, 8)}...`);
    }
  }
      ...
      try {
        const resp = await fetch(url.toString(), {
          headers: { 
            ...
            "Ocp-Apim-Subscription-Key": this.apiKey  // ← Using your SPORTSDATA_API_KEY here
          },
        });
  ...
  async getMLBPlayerGameLogs(playerName, season = null, count = 15) {
    try {
     ...
     if (!season) {
  console.log(`[getMLBPlayerGameLogs] ⚠️ SEASON IS NULL - CALLING AUTO-DETECT`);
  season = await _detectMLBSeason(this);
  console.log(`[getMLBPlayerGameLogs] ⚠️ AUTO-DETECT RETURNED: ${season}`);
        console.log(`[getMLBPlayerGameLogs] Auto-detected season: ${season}`);
      }
      
      console.log(`[getMLBPlayerGameLogs] Fetching season stats for ${season}...`);
      ...
      console.log(`[getMLBPlayerGameLogs] Searching for player: ${playerName} in ${seasonStats.length} players`);
      ...
      console.log(`[getMLBPlayerGameLogs] Fetched ${gameLogs.length} total game records`);
